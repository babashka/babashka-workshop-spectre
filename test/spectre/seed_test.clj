...
(deftest seed-test
  (fs/with-temp-dir [dir {}]
    (let [opts {:path (str (fs/path dir "db.edn"))}]
      (testing "a missing database gets example sites"
        (seed/seed! opts)
        (is (= {:sites seed/sites} (db/load-db opts))))
      (testing "existing entries and other database fields are preserved"
        (let [existing {:sites {"google.com" {:counter 9}
          ...
          (is (= (update existing :sites #(merge seed/sites %))
                 (db/load-db opts)))))
      (testing "repeating the task does not write the database"
